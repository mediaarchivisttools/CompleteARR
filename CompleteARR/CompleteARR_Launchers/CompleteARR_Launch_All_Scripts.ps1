...
<#
    ...
    Orchestrates the CompleteARR suite for BOTH Radarr and Sonarr:
      ...
      2) Runs Radarr AutoSorter
      3) Runs Radarr Film Engine
      4) Runs Sonarr AutoSorter
      5) Runs Sonarr Series Engine
...
$RadarrAutoSorterPath = Join-Path $CompleteArrRoot 'CompleteARR_Scripts\CompleteARR_RADARR_AutoSorter.ps1'
$RadarrEnginePath     = Join-Path $CompleteArrRoot 'CompleteARR_Scripts\CompleteARR_RADARR_FilmEngine.ps1'
$SonarrAutoSorterPath = Join-Path $CompleteArrRoot 'CompleteARR_Scripts\CompleteARR_SONARR_AutoSorter.ps1'
...
if (-not (Test-Path -LiteralPath $RadarrAutoSorterPath)) {
    Write-Host "ERROR: Could not find Radarr AutoSorter script at:" -ForegroundColor Red
    Write-Host "  $RadarrAutoSorterPath" -ForegroundColor Red
    $allScriptsExist = $false
}
...
if (-not (Test-Path -LiteralPath $SonarrAutoSorterPath)) {
    Write-Host "ERROR: Could not find Sonarr AutoSorter script at:" -ForegroundColor Red
    Write-Host "  $SonarrAutoSorterPath" -ForegroundColor Red
    $allScriptsExist = $false
}
...
Write-Host " 🔄 STEP 2: Running Radarr AutoSorter" -ForegroundColor Yellow
Write-Host "------------------------------------------------" -ForegroundColor Yellow
Write-Host ""
Write-Host "📋 The Radarr AutoSorter will:" -ForegroundColor Yellow
...
$radarrAutoSorterSummary = $null
try {
    $radarrAutoSorterSummary = & $RadarrAutoSorterPath
    Write-Host ""
    Write-Host "✅ Radarr AutoSorter completed successfully!" -ForegroundColor Green
    ...
    Write-Host "  $($_.Exception.Message)" -ForegroundColor Red
    Write-Host ""
    Write-Host "WARNING: Continuing to run Radarr Engine anyway." -ForegroundColor Yellow
}

Write-Host ""
Write-Host "------------------------------------------------" -ForegroundColor Yellow
Write-Host " 🔄 STEP 3: Running Radarr Film Engine" -ForegroundColor Yellow
Write-Host "------------------------------------------------" -ForegroundColor Yellow
Write-Host ""
...
Write-Host "------------------------------------------------" -ForegroundColor Yellow
Write-Host " 🔄 STEP 4: Running Sonarr AutoSorter" -ForegroundColor Yellow
Write-Host "------------------------------------------------" -ForegroundColor Yellow
Write-Host ""
Write-Host "📋 The Sonarr AutoSorter will:" -ForegroundColor Yellow
...
$sonarrAutoSorterSummary = $null
try {
    $sonarrAutoSorterSummary = & $SonarrAutoSorterPath
    Write-Host ""
    Write-Host "✅ Sonarr AutoSorter completed successfully!" -ForegroundColor Green
    ...
    Write-Host "  $($_.Exception.Message)" -ForegroundColor Red
    Write-Host ""
    Write-Host "WARNING: Continuing to run Sonarr Engine anyway." -ForegroundColor Yellow
}

Write-Host ""
Write-Host "------------------------------------------------" -ForegroundColor Yellow
Write-Host " 🔄 STEP 5: Running Sonarr Series Engine" -ForegroundColor Yellow
Write-Host "------------------------------------------------" -ForegroundColor Yellow
Write-Host ""
...
# Radarr AutoSorter Summary
if ($radarrAutoSorterSummary) {
    Write-Host "🎥 RADARR AUTOSORTER:" -ForegroundColor Magenta
    Write-Host ("  Movies seen total          : {0}" -f $radarrAutoSorterSummary.MoviesSeenTotal) -ForegroundColor White
    Write-Host ("  Movies in source profiles  : {0}" -f $radarrAutoSorterSummary.MoviesInSourceProfiles) -ForegroundColor White
    Write-Host ("  Movies moved               : {0}" -f $radarrAutoSorterSummary.MoviesMoved) -ForegroundColor White
    Write-Host ("  Movies already correct     : {0}" -f $radarrAutoSorterSummary.MoviesAlreadyCorrect) -ForegroundColor White
    Write-Host ("  Movies skipped (no target) : {0}" -f $radarrAutoSorterSummary.MoviesSkippedNoTarget) -ForegroundColor White
    Write-Host ""
}
...
# Sonarr AutoSorter Summary
if ($sonarrAutoSorterSummary) {
    Write-Host "🎬 SONARR AUTOSORTER:" -ForegroundColor Blue
    Write-Host ("  Series seen total          : {0}" -f $sonarrAutoSorterSummary.SeriesSeenTotal) -ForegroundColor White
    Write-Host ("  Series in source profiles  : {0}" -f $sonarrAutoSorterSummary.SeriesInSourceProfiles) -ForegroundColor White
    Write-Host ("  Series moved               : {0}" -f $sonarrAutoSorterSummary.SeriesMoved) -ForegroundColor White
    Write-Host ("  Series already correct     : {0}" -f $sonarrAutoSorterSummary.SeriesAlreadyCorrect) -ForegroundColor White
    Write-Host ("  Series skipped (no target) : {0}" -f $sonarrAutoSorterSummary.SeriesSkippedNoTarget) -ForegroundColor White
    Write-Host ""
}
